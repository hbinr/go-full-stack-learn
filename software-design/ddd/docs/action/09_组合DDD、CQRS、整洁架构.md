...
从事一个编程项目类似于规划和建设一个住宅区。如果你知道该区在不久的将来会扩大，你需要为未来的改进保留空间。即使在开始的时候，这可能看起来是对空间的浪费。你应该为未来的设施如住宅区、医院和寺庙保留空间。如果没有这一点，你将被迫摧毁建筑物和街道，为新建筑腾出空间。早点考虑到这一点会好很多。

代码的情况也是如此。如果你知道项目的开发时间将超过1个月，你就应该从一开始就把长期的事情放在心上。你需要以一种不会阻碍你未来工作的方式来创建你的代码。即使一开始看起来是过度工程和大量额外的模板，你也需要牢记长期的目标。

这并不意味着你需要规划你将来要实现的每一个功能--实际上正好相反。这种方法有助于适应新的需求或对我们领域不断变化的理解。这里不需要大的前期设计。这在目前的时代是很关键的，因为世界变化非常快，谁不能适应这些变化，谁就会被淘汰。

这正是这些模式结合起来给你带来的东西--保持稳定的开发速度的能力。在不破坏和过多触及现有代码的情况下。

是否需要更多的思考和计划？是否是更有挑战性的方式？你需要有额外的知识来做到这一点吗？当然！但长期的结果是值得的。幸运的是，你在正确的地方可以学到这些。

但让我们把理论抛在脑后。让我们去看看代码。在这篇文章中，我们将跳过对我们的设计选择的推理。我们在以前的文章中已经描述了这些。如果你还没有读过这些文章，我建议你读一下--你会更好地理解这篇文章。
...
图片![](https://cdn.nlark.com/yuque/0/2021/png/2774323/1629114836529-24c1481c-266a-4e52-b5b8-0ea0042d860d.png?x-oss-process=image%2Fresize%2Cw_833)
...
批准领域层中的重新安排
正如DDD Lite介绍中所述，我们用面向行为的方法构建我们的领域。而不是在数据上。让我们在我们的领域实体上建立ApproveReschedule模型。
```go
package training
...
### Orchestrate with command 用命令进行rchestrate
...
## Refactoring of training cancelation 培训取消的重构
...
现在让我们来看看TrainingService的CancelTraining。

这里的领域逻辑很简单：你可以在培训日期前24小时内取消培训。如果在培训前不到24小时，而你还是想取消培训。

- 如果你是培训师，参会者将得到他的培训 "回报"，外加一次额外的课程（没有人喜欢在同一天改变计划！）。
- 如果你是参会者，你将失去这次培训。
这就是目前的实施情况：
```go
...
像这样的逻辑应该生活在我们的领域层中。如果你开始看到一些与应用层中的逻辑有关的if，你应该考虑如何将它移到领域层。这将更容易测试和在其他地方重新使用。

这可能取决于项目，但通常领域逻辑在最初的开发之后是相当稳定的，可以长期保持不变。它可以在服务、框架变化、库变化和API变化之间生存。由于这种分离，我们可以以更安全、更快速的方式进行所有这些改变。

让我们把CancelTraining方法分解成多个独立的部分。这将使我们能够独立地测试和改变它们。
...
第二个需要移动的部分是取消后计算训练余额的 "算法"。理论上，我们可以把它放在Cancel()方法中，但我认为这将破坏单一责任原则和CQS。而且我喜欢小函数。
...
1.从资源库中获取实体。
2.协调域的东西。
3.调用外部培训师服务来取消培训（这个服务是 "培训师日历 "的真实点）。
4. 返回实体以保存在数据库中。
...
由于引入了training.Training实体，我们可以有一个更简单的版本，用一个方法来添加一个新的训练，一个方法来更新。
...
在上一篇文章中，我们使用Firestore实现了我们的存储库。我们在当前的实现中也将使用Firestore。请记住，这是一个实现细节--你可以使用任何你想要的数据库。在之前的文章中，我们已经展示了使用不同数据库的实例实现。
...
连接一切
现在如何使用我们的代码？我们的端口层怎么样了？由于 Miłosz 在重构清洁架构一文中所做的重构，我们的端口层已经与其他层解耦。这就是为什么，在这次重构之后，它几乎不需要任何重大改变。我们只是调用应用程序命令而不是应用程序服务。
...
在这种情况下，你不应该考虑Mob/Pair编程的时间损失。你应该考虑的是你可能因为不做而损失的时间。这也会帮助你更快地完成重构，因为你不需要等待决定。你可以立即就它们达成一致。

在实施复杂的绿地项目时，Mob和结对编程也能完美地发挥作用。在这种情况下，知识共享是特别重要的投资。我曾多次看到这种方法是如何让项目在长期内快速发展的。
...
但是，什么时候才能知道项目是否复杂到可以使用暴徒编程？不幸的是，这并没有什么神奇的公式。但有一些问题你应该问自己。

我们是否了解这个领域？
我们知道如何实现它吗？
最终会不会产生一个没有人能够审查的巨大的拉动请求？
我们能不能在不做暴徒/配对编程的同时冒着更大的知识共享风险？
总结
我们就这样结束了。😄

整个重构的diff可以在我们的Wild Workouts GitHub上找到（注意，它很庞大！）。
