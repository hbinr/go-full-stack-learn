 ...
 * var let 声明变量，内容要点：
 * 1. ES5中，var的弊端介绍和案例
 * 2. ES6中，新关键字let学习和案列，为了解决var存在的问题
 * 3. let 用于定义变量
 * 4. 块级作用域：用大括号 {} 包裹的变量便处于块级作用域内，常见有：
 *  a. {}  裸大括号
 *  b. if、 else、 for、 while 语句中
 *  c. try、 catch  语句中
 *      
 */
...
console.log('------------------');
// 下面这行代码会输出 i=5，正常情况应该是编译报错 undefined: i，这才是正常逻辑，如Go，Java都是这样处理的
...
{
    var e = 30 // 块级作用域，用大括号包裹便处于块级作用域。 
}
// 执行这行代码时，正常情况应该是编译报错 undefined: e，这才是正常逻辑
console.log('e: ', e);  // e: 30，不会报错，而且还有值。此示例在go语言中会编译报错： e declared but not used
// go语言中会编译报错：  undefined: e
